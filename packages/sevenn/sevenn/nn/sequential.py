...
class AtomGraphSequential(nn.Sequential):
    ...
    def forward(self, input: AtomGraphDataType) -> AtomGraphDataType:
        # [PROFILING] Import here to avoid circular import and minimize overhead
        from .profiling import is_profiling_enabled, record_function_if_enabled
        
        data = self._preprocess(input)
        
        if is_profiling_enabled():
            # [PROFILING] Track each module individually
            for name, module in self._modules.items():
                with record_function_if_enabled(f"SevenNet::{name}"):
                    data = module(data)
        else:
            for module in self:
                data = module(data)
